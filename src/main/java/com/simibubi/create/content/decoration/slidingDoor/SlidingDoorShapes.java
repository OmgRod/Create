...
public class SlidingDoorShapes {
    ...
    public static VoxelShape get(Direction facing, boolean hinge, boolean fold) {
        if (fold) {
            switch (facing) {
                case SOUTH:
                    return hinge ? ES_AABB_FOLD : WS_AABB_FOLD;
                case WEST:
                    return hinge ? SW_AABB_FOLD : NW_AABB_FOLD;
                case NORTH:
                    return hinge ? WN_AABB_FOLD : EN_AABB_FOLD;
                default:
                    return hinge ? NE_AABB_FOLD : SE_AABB_FOLD;
            }
        }

        switch (facing) {
            case SOUTH:
                return hinge ? ES_AABB : WS_AABB;
            case WEST:
                return hinge ? SW_AABB : NW_AABB;
            case NORTH:
                return hinge ? WN_AABB : EN_AABB;
            default:
                return hinge ? NE_AABB : SE_AABB;
        }
    }
}
